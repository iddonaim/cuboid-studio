...
| **POETIKS / פה־אתיקה** | The current thesis title, adopted 2026-07-23 (D1). ⚠ A **working name, not locked** — the final lock is still deferred, so treat it as provisional in anything that has to survive review. See `docs/THESIS_QA_AGENT_SCOPE.md` §6. The claim behind it: architecture can be produced through a translation pipeline where memes supply the cultural layer, site data supplies the ground truth, an LLM mediates, and the architect curates. |
| **Topological Translation** | ⚠ **Retired 2026-07-23.** Not a rebrand — the old titles were cleared before a new one existed. The same-day §E correction dropped "Topological Translation", "MemeMachine" and "memescape" together and recorded that the project had no name yet. POETIKS / פה־אתיקה was adopted later that day as the working name (D1), final lock still deferred. Kept as an entry so older material stays readable: anything dated before 2026-07-23 — the v1 prompts in `src/prompts/`, archived spec text, earlier thesis drafts — uses this name and means POETIKS. |
...
- **Connection rules** — The logic deciding which cube faces may join. Each face carries a **set** of cut types (sphere, cylinder, or neither); two faces join when their sets **intersect**, and an uncut face — a shell, the empty set — joins nothing. So sphere↔sphere and cylinder↔cylinder connect, sphere↔cylinder doesn't, and a face carrying *both* joins either. Marketing tagline: "They only connect where their cuts agree." ⚠ The door/window gloss is an interpretation laid over the cutter geometry — the rule reads cutters. ⚠ Until 2026-07-28 the derivation recorded one face per sphere and two per cylinder and was wrong for 69 of 70 variations; see `docs/GAPS_AND_HOLES.md` P0-6.
