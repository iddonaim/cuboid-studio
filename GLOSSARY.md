...
- **v1 vs v2** — v1 (single-pass) mapped how a meme *looks* onto geometry; v2 explicitly repudiates that and translates what the meme *does* culturally (content + affect + site). Both prompts remain in the repo as before/after evidence. Evolve still runs on v1 single-pass.
- **Pass mode** — Per-request switch between single-pass and two-pass. Defaults to two-pass in the UI; Evolve always sends single-pass. (The `TRANSLATION_PASS_MODE` env var is dead — nothing reads it.)
...
- **Magnitude / decay** — Numeric parameters of a translation (how strong the move is; how sharply it falls off). ⚠ Both are currently inert at runtime — stored and displayed, but they don't modulate the geometry.
