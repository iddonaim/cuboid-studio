...
| **POETIKS / פה־אתיקה** | The formal thesis title, adopted 2026-07-23. The claim: architecture can be produced through a translation pipeline where memes supply the cultural layer, site data supplies the ground truth, an LLM mediates, and the architect curates. ⚠ Still a **working name, not locked** — see `docs/THESIS_QA_AGENT_SCOPE.md` §6. |
| **Topological Translation** | ⚠ **Retired title.** The thesis was called this until 2026-07-23, when it was renamed to **POETIKS / פה־אתיקה**. Kept as an entry so older material stays readable: anything dated before 2026-07-23 — the v1 prompts in `src/prompts/`, archived spec text, earlier thesis drafts — uses this name and means POETIKS. ⚠ The rationale for dropping it is **not recorded anywhere in this repo** — Iddo to fill in, since reviewers will ask. |
