 ...
 *
 * axisPosition is taken directly from the GH plane origin (the 2 coordinates
 * perpendicular to the cylinder's axis), verified against GH Area component centroids.
 * Earlier versions incorrectly added a local 2D plane offset to the origin.
 */

export const CYLINDER_05: CylinderSpec = {
  id: 4,
  type: 'cylinder',
  // Axis position taken directly from GH plane origin (Area centroid verified)
  // Plane origin: (4.90, 0.00, -1.20) → axisPosition = [X, Z] = [4.90, -1.20]
  ...
  notes: 'Runs along Y-axis; axis passes through X=4.90, Z=-1.20'
};
...
export const CYLINDER_06: CylinderSpec = {
  ...
  type: 'cylinder',
  // Axis position taken directly from GH plane origin (Area centroid verified)
  // Plane origin: (42.00, -1.80, 27.80) → axisPosition = [Y, Z] = [-1.80, 27.80]
  ...
  notes: 'Runs along X-axis (negative direction); axis passes through Y=-1.80, Z=27.80'
};
...
export const CYLINDER_07: CylinderSpec = {
  ...
  type: 'cylinder',
  // Axis position taken directly from GH plane origin (Area centroid verified)
  // Plane origin: (30.60, 42.00, 26.30) → axisPosition = [X, Z] = [30.60, 26.30]
  ...
  extrusionVector: [0, -51, 0],
  notes: 'Runs along Y-axis (negative direction); axis passes through X=30.60, Z=26.30'
};
 ...
 * | 4  | Cylinder | X=4.90,  Z=-1.20, axis Y   | 16.18034  | Along +Y       |
 * | 5  | Cylinder | Y=-1.80, Z=27.80, axis X   | 9.864601  | Along -X       |
 * | 6  | Cylinder | X=30.60, Z=26.30, axis Y   | 13        | Along -Y       |
