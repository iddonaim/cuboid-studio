...
const inputCls = "w-full box-border px-2 py-1.5 font-[inherit] text-[11px] text-[#e0e0e0] bg-slate-800 border border-slate-700 rounded outline-none focus:border-cyan-700";
...
const Label: React.FC<{ text: string; sub?: string }> = ({ text, sub }) => (
  <label className="block text-[10px] font-semibold text-slate-400 mb-1">
    {text}
    {sub && <span className="block font-normal text-[9px] text-slate-500 mt-px">{sub}</span>}
  </label>
);

const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="mt-4">
    <div className="text-[10px] font-semibold text-cyan-400 mb-2 pb-1 border-b border-slate-700 tracking-[0.05em] uppercase">
      {title}
    </div>
    {children}
  </div>
...
export const SiteContextCurator: React.FC<SiteContextCuratorProps> = ({ open, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-950 rounded-xl border border-slate-800 w-[90vw] max-w-[640px] max-h-[85vh] flex flex-col overflow-hidden"
      ...
      >
        ...
        <div className="flex justify-between items-center px-5 py-4 border-b border-slate-800">
          <div>
            ...
            <h2 className="text-white text-base font-semibold mt-1">Site Context Curator</h2>
          </div>
          <button
            onClick={onClose}
            className="bg-transparent border-0 text-slate-400 text-2xl cursor-pointer leading-none"
          >
            &times;
          </button>
        </div>
        ...
        <div className="px-5 py-2 border-b border-slate-800">
          <input
            ...
            className="w-full box-border bg-transparent border-0 border-b border-slate-700 rounded-none px-0 py-1 text-[13px] font-medium text-cyan-400 outline-none font-[inherit]"
          />
        </div>

        {/* Tabs */}
        <Tabs value={tab} onValueChange={setTab} className="flex flex-col flex-1 overflow-hidden">
          <TabsList className="w-full rounded-none h-auto p-0 bg-transparent border-b border-slate-800 justify-start">
            {[
          ...
          <div className="flex-1 overflow-y-auto px-5 py-3 pb-6">
                ...
                <div className="mb-2">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <button
                      onClick={geocode}
                      disabled={geocoding}
                      className={`px-2.5 py-1.5 bg-slate-800 border border-slate-700 rounded text-[10px] whitespace-nowrap cursor-pointer ${geocoding ? 'text-slate-600 cursor-wait' : 'text-cyan-400'}`}
                    >
                      {geocoding ? '...' : 'GEOCODE'}
                    </button>
                  </div>
              ...
              <Section title="Sun & Wind">
                <div className="flex justify-between items-center mb-2">
                  <button
                    ...
                    className={`px-2 py-1 rounded text-[9px] cursor-pointer border ${autoSun ? 'bg-emerald-950 border-green-700 text-green-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}
                  >
                    {autoSun ? 'Auto sun ON' : 'Auto sun OFF'}
                  </button>
                  {sunOk && <span className="text-[9px] text-green-500">computed</span>}
                </div>
                <div className="mb-2">
            ...
            <TabsContent value="programmatic" className="mt-0">
              <Section title="Existing Uses">
                {p.existing_uses.map((item, i) => (
                  <div key={i} className="bg-slate-800 border border-slate-700 rounded-md p-3 mb-2 relative">
                    {p.existing_uses.length > 1 && (
                      <button
                        onClick={() => setP(prev => ({ ...prev, existing_uses: prev.existing_uses.filter((_, j) => j !== i) }))}
                        className="absolute top-1.5 right-2 bg-transparent border-0 text-slate-600 cursor-pointer text-sm"
                      >
                        &times;
                      </button>
                    )}
                    ...
                    <div className="flex gap-3 mb-1.5">
                      <label className={`text-[10px] flex items-center gap-1 cursor-pointer ${item.formal ? 'text-green-500' : 'text-slate-500'}`}>
                      ...
                      <label className={`text-[10px] flex items-center gap-1 cursor-pointer ${!item.formal ? 'text-red-400' : 'text-slate-500'}`}>
                ...
                <button
                  ...
                  className="w-full py-2 bg-transparent border border-dashed border-slate-700 rounded text-slate-500 cursor-pointer text-[10px]"
                >
                  + Add existing use
                </button>
              </Section>
            ...
            <TabsContent value="reading" className="mt-0">
              <div className="bg-slate-800 border border-slate-700 rounded-md p-2.5 mb-4 text-[10px] text-amber-400 leading-relaxed">
            ...
            <TabsContent value="export" className="mt-0">
              <div className="mb-4">
                <pre className="bg-slate-950 border border-slate-700 rounded-md p-3 text-[10px] text-slate-400 leading-relaxed overflow-auto max-h-[300px] whitespace-pre-wrap break-words">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={handleSave}
                  className={`flex-1 h-auto py-2.5 text-xs font-semibold text-white border-0 ${saved ? 'bg-green-600 hover:bg-green-500' : 'bg-emerald-900 hover:bg-emerald-800'}`}
                >
                  {saved ? 'Saved!' : 'Save as Active Context'}
                </Button>
                <Button
                  onClick={handleClear}
                  className="h-auto py-2.5 px-4 text-xs bg-red-900 hover:bg-red-800 text-white border-0"
                >
                  Clear
                </Button>
              </div>
