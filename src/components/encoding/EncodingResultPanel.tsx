...
export const EncodingResultPanel: React.FC<EncodingResultPanelProps> = ({ docked = false }) => {
  ...
  return (
    ...
    >
      ...
      {encodingReasoning ? (
        <div className="p-2 bg-ink-100 border border-ink-200 rounded text-ink-700 text-[11px] leading-relaxed italic">
          {encodingReasoning}
          {mode !== 'standalone' && seedCubes.length > 0 && (
            <p className="text-ink-600 text-[11px] mt-1.5 mb-0 not-italic">
              {seedCubes.length} preserved · {encodedCubes?.length ?? 0} added
            </p>
          )}
        </div>
      ) : (
        <p className="text-ink-500 text-[11px] m-0">No interpretation text returned.</p>
      )}
    </div>
  );
};
