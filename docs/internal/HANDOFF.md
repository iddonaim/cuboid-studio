...
- **Evolution / Pataphysical** (live): v1 single-pass and v2 two-pass both wired to UI; default `passMode = 'two_pass'`, user-toggleable. Operators: inversion/amplification/drift/reassignment/preservation/shuffle (v1) + consolidation/erosion/reinforcement (v2). 4-axis confidence vector. OpenRouter default (`anthropic/claude-sonnet-4.6`), Anthropic fallback.
