...
| **Decode** | Live | 2D notation sheet (Konva) as the full-bleed stage. Drag/place/rotate tile glyphs of variations, snap-to-grid, export as SVG or DXF. The 3D assembly is reference material behind a Sheet/3D switch. |
...
- **Corner preview (`DecodeAssemblyPreview.tsx`):** the assembly as a small live 3D thumbnail on the sheet, bottom-right (the stage's left is under the docked sidebar). Its own `<Canvas>` with a throwaway camera fitted to the assembly bounds — resizing the real `Viewport3D` into a thumbnail drags its camera state, orbit controls and capture registration along and returns it framed for a 240px box. Clicking it opens the full 3D, which is why the stage toggle only mounts over the 3D (as "Sheet") and not over the sheet.
