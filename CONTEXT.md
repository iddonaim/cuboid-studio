...
- **Export (`decodeSheetExport.ts`):** three formats, because DXF carries geometry and nothing else — no layers, no groups, no names.
  - **SVG and PNG carry every layer** (2026-08-08): the underlay stack exports with the tiles. A registered plan or capture is part of the drawing, not chrome — what leaves Decode is what the sheet shows. Only *hidden* layers and genuine editing chrome are dropped. The crop (`sheetBounds`) is measured from tiles **and** visible underlays together, rotated footprints included, so a plan reaching past the tiles is framed rather than sliced; and with a layer registered, export is available even with no tiles placed. DXF is the sole exception — the format cannot carry a raster image.
  - **SVG** — the sheet as editable vectors. Each variation is inlined once as a `<symbol>` and each tile a `<use>` inside a named `<g id="tile-N-v-XX">`, so a drawing arrives as a selectable list rather than one welded blob. Every variation file defines its own `.cls-1`, `.cls-2`… so class names are rewritten per variation before combining — without that, the last definition silently repaints every earlier tile. Underlays ride along as one `<image>` per layer inside `<g id="underlay-N-label">`, beneath the tiles and painted bottom-up, each at its own registration and opacity — so the layers stay **separable in Illustrator** rather than baked in. The image is embedded as a base64 data URI (self-contained file, at the cost of size: a full-res plan can add megabytes) via `xlink:href`, which Illustrator and older drawing programs still read where the SVG2 `href` alone may not.
  - **PNG** — transparent cut-out off the live Konva stage at 3×, flattening tiles and visible underlays together. Only the drafting chrome (grid, selection, interactive snap dots, and the armed layer's `Transformer` handles) is hidden via a `sheet-chrome` node name; the stage is momentarily reset to 1:1 at the drawing's corner so the crop is predictable, then restored. Registered/called through the same registry shape the 3D viewport's capture uses.
  - **DXF** (`decodeDxfExport.ts`, via `dxf-writer`) — tile geometry only, no underlay images.
  - Guarded end-to-end in `e2e/decode-export.spec.ts`: a solid-colour plan is imported through the real UI, exported, and the resulting pixels are checked for the plan, the tile line work, and the *absence* of transform handles.
...
- **Corner preview (`DecodeAssemblyPreview.tsx`):** the assembly as a small live 3D thumbnail on the sheet, bottom-right (the stage's left is under the docked sidebar). Its own `<Canvas>` — resizing the real `Viewport3D` into a thumbnail drags its camera state, orbit controls and capture registration along and returns it framed for a 240px box. Clicking it opens the full 3D, which is why the stage toggle only mounts over the 3D (as an accent "← Notation sheet" pill, top-centre under the TopBar) and not over the sheet. It used to be a grey Ortho-sized pill at `bottom-[132px] right-3` — the Ortho toggle's exact coordinates, which it covered.
...
- **Underlay stack (up to 4):** raster plans *and* saved viewport captures sit beneath the tiles as a stack — index 0 is the top, matching the layers list. Each layer carries its own registration (offset / rotation / scale), visibility and opacity (`DecodeUnderlay` in `useDecodeStore`). *Use in Decode* from a capture **appends** rather than replacing, which is the point: a captured 3D view registered over a site plan.
  ...
  - **They export.** A registered layer is part of the drawing and leaves with it in SVG and PNG — see Export above.
  - **They save.** Tiles, layers and registration all ride in the composition and come back on load — this was already true, and the sidebar now says so instead of leaving the sheet looking export-only.
...
- **A composition shows its drawing** (2026-08-08): the snapshot carries a ~240px PNG of the notation sheet (`decode.sheetThumbnailDataUrl`), rendered on the composition row so saves are told apart by what they *are* rather than by name and date. PNG, not JPEG — the sheet is a transparent cut-out and JPEG would flatten it onto black. Konva can only photograph a **mounted** stage while "Save to project" is reachable from any mode, so `DecodeCanvas` refreshes the thumbnail into the decode store on a 600ms debounce after the drawing settles (deliberately *not* on unmount, where React's teardown ordering may already have disposed the stage); `captureComposition()` prefers a fresh capture and falls back to that cached one. Measured 7–14KB in practice against a 150KB ceiling, and `captureSheetThumbnail` steps 240px → 160px before giving up — a composition saves without a thumbnail rather than failing the write. At thumbnail scale tile line work over a large plan is necessarily faint; that is what the drawing looks like small, not a defect.
- **Restoring returns you to where the work was** (2026-08-08): `savedFromMode` records the active mode at save time and `landingModeFor()` sends a sheet-saved composition back to Decode instead of always landing in Encode whenever an assembly exists. Compositions saved before the field existed keep the old inference (Decode only when it holds the only meaningful content), and an empty sheet never wins regardless.
